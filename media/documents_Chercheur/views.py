...
def division(request,pk):
    ...
    if len(hell) > 0: #random avec choix
        ...
        while (len(list_articl) > 0) : # rest articles a distribuer
            ...
            #if j < 4 :               # ajouter evaluateur choix avec eval non choix ---> meme niveau (nb articles)
            print('niveau',j)
            ...
            for ev in new_liste_eval:
                print('eval ', ev  ,' nivau ',ev.artcl_a_corrige.count())
                if ev.artcl_a_corrige.filter(Conferance = conferance).count() == j:
                    list_evalu_ini.append(ev)

            
            for k in hell:
                print('eval ',k.evalu  ,' nivau ',k.evalu.artcl_a_corrige.count())
                if k.evalu.artcl_a_corrige.filter(Conferance = conferance).count() == j :      # list eval meme niveau
                    list_evalu_ini.append(k.evalu)
                    #ele = hell.index(k)
            ...
            j += 1  
            print('incrementation niveau a ',j)
